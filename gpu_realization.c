...
cl_int calculate_host(cl_context context, cl_command_queue command_queue,
  ...
  if (rounded_n > LOCAL2 * 2) {
    error = calculate_host(
        context, command_queue, up_and_down_sweep, sums_mem,
        round_to_div(rounded_n, LOCAL2 * 2), events, event_i, ans);
    if (error != CL_SUCCESS) {
      clReleaseMemObject(sums_mem);
      return 1;
    };
  } else {
    error = clEnqueueReadBuffer(command_queue, sums_mem, CL_TRUE, 0, sizeof(cl_float) * 1, ans,
...
int calculate1(cl_device_id device_id, cl_float *a, size_t n, cl_float *ans) {
  ...
  cl_kernel up_and_down_sweep =
      clCreateKernel(program, "reduce", &error);
